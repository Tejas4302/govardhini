...
const CreateUserForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: existingUser
      } = await supabase.from('users').select('phone_number').eq('phone_number', formData.phoneNumber).single();
      if (existingUser) {
        toast({
          title: "Error",
      ...
      <SheetContent className="glass-card border-emerald-500/20 bg-gradient-to-br from-emerald-500/5 to-green-500/5 text-white w-[400px] sm:w-[540px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-6">
          <div className="space-y-2">
            ...
            <Input id="fullName" type="text" placeholder="Enter full name" value={formData.fullName} onChange={e => setFormData(prev => ({
            ...prev,
            fullName: e.target.value
          }))} className="glass-input border-emerald-500/30 text-white placeholder:text-emerald-300" disabled={isLoading} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="phoneNumber" className="text-emerald-200">Phone Number</Label>
            <Input id="phoneNumber" type="tel" placeholder="Enter 10-digit phone number" value={formData.phoneNumber} onChange={e => setFormData(prev => ({
            ...prev,
            phoneNumber: e.target.value
          }))} className="glass-input border-emerald-500/30 text-white placeholder:text-emerald-300" disabled={isLoading} maxLength={10} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="designation" className="text-emerald-200">Designation</Label>
            <Select value={formData.designation} onValueChange={value => setFormData(prev => ({
            ...prev,
            designation: value
          }))} disabled={isLoading}>
              ...
              <SelectContent className="glass-card border-emerald-500/20 bg-slate-800">
                {availableRoles.map(role => <SelectItem key={role} value={role} className="text-white hover:bg-emerald-500/20">
                    {role}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={generatePassword} className="text-emerald-400 hover:text-emerald-300 text-xs" disabled={isLoading}>
                Generate
              </Button>
            </div>
            <div className="relative">
              <Input id="password" type={showPassword ? "text" : "password"} placeholder="Enter password" value={formData.password} onChange={e => setFormData(prev => ({
              ...prev,
              password: e.target.value
            }))} className="glass-input border-emerald-500/30 text-white placeholder:text-emerald-300 pr-12" disabled={isLoading} />
          ...
          <div className="flex gap-2 pt-4">
            <Button type="button" variant="outline" onClick={onClose} disabled={isLoading} className="flex-1 border-emerald-500/30 text-emerald-200 bg-gray-700 hover:bg-gray-600">
              Cancel
            </Button>
            <Button type="submit" disabled={isLoading} className="flex-1 grass-green hover:bg-emerald-700 text-white">
