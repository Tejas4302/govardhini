...
const UserManagement = () => {
  ...
  return <AdminGuard>
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-green-900 to-emerald-900">
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center">
                <Button variant="ghost" onClick={() => navigate('/dashboard')} className="mr-4 text-emerald-300 hover:text-emerald-100 hover:bg-emerald-500/20">
                  <ArrowLeft className="w-5 h-5 mr-2" />
                  Back to Dashboard
                </Button>
                <h1 className="font-bold text-white animate-fade-in text-2xl">User Management</h1>
              </div>
            </div>
            ...
            <Card className="glass-card border-emerald-500/20 bg-gradient-to-br from-emerald-500/5 to-green-500/5 animate-fade-in">
              ...
              <CardContent>
                {isLoading ? <div className="text-center text-white py-8">
                  ...
                  </div> : users.length === 0 ? <div className="text-center text-emerald-300 py-8">
                    <Users className="w-16 h-16 mx-auto mb-4 opacity-50" />
                    <p>No users found</p>
                  </div> : <div className="glass-card border-emerald-500/20 bg-emerald-500/5 rounded-lg overflow-hidden">
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {users.map((userData, index) => <TableRow key={userData.id} className="border-emerald-500/10 hover:bg-emerald-500/10 transition-colors animate-slide-up" style={{
                      animationDelay: `${index * 0.1}s`
                            ...
                            <TableCell>
                              {userData.id !== user.id && <div className="flex gap-1">
                                  <RoleChangeAction user={userData} availableRoles={availableRoles} processingUserId={processingUserId} onRoleChange={handleRoleChange} />
                            ...
                            if (!open) setUserToDelete(null);
                                    ...
                                    <SheetTrigger asChild>
                                      <Button size="sm" variant="ghost" disabled={processingUserId === userData.id} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 border-0 bg-transparent" onClick={() => setUserToDelete(userData)} style={{
                                width: '32px',
                                height: '32px',
                                minWidth: '32px',
                                minHeight: '32px',
                                ...
                                justifyContent: 'center'
                                  ...
                                  setUserToDelete(null);
                                        ...
                                        <Button onClick={() => deleteUser(userData.id, userData.full_name)} className="bg-red-700 hover:bg-red-800">
                                          Delete Account
                                        </Button>
                                      </SheetFooter>
                                    </SheetContent>
        ...
        <CreateUserForm isOpen={showCreateUserForm} onClose={() => setShowCreateUserForm(false)} onUserCreated={fetchUsers} />
      </div>
    </AdminGuard>;
};
